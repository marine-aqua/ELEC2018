...
由 $y[-1]=y[-2]=0$ 定齐次系数：
\[
y_{zs}[n]=\left[2n\left(\frac{1}{2}\right)^n-2\left(\frac{1}{2}\right)^n+2\left(\frac{1}{4}\right)^n\right]u[n].
\]
全响应
\[
y[n]=y_{zi}+y_{zs}=\left[(2n-1)\left(\frac{1}{2}\right)^n+\frac{7}{4}\left(\frac{1}{4}\right)^n\right]u[n].
\]
...
\textbf{(c)} $Y(z)=\dfrac{z^2}{(z-1)(z-0.6)^2}$，$y[n]$ 含 $(0.6)^n$ 与常数项；因 $|0.6|<1$，$y[\infty]=1$（终值定理 $y[\infty]=\lim_{z\to1}(z-1)Y(z)=1$）。
